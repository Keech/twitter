...
define('FAVORITE_HISTORY_PREFIX', 'favhistory-');
...
class Common extends Core {
	...
	protected $FAVORITE_HISTORY_FILE_PATH = '';

	/**
	 * 使用するアカウントのconfigをセットする
	 *
	 * @param string $conf_nm 使用するconfigファイル名から拡張子を除いたもの
	 */
	protected function set_config($conf_nm){
		...
		$this->config = $config;
		$this->config['follow_keyword_today'] = $this->get_todays_keyword($this->config['follow_keyword']);
		$this->config['fav_keyword_today'] = $this->get_todays_keyword($this->config['fav_keyword']);
	}
	...
	protected function __construct($conf_nm){
		...
		$this->FAVORITE_HISTORY_FILE_PATH = __DATA_DIR__.DS.FAVORITE_HISTORY_PREFIX.$user_screen_name;
	}

	/**
	 * 日付をキーとしてconfigに設定されたキーワード配列から実際に検索するキーワードを決定する
	 *
	 * @param array $keywords configで設定されたキーワード群の配列
	 * @return string 日付をキーにして選ばれたキーワード
	 */
	public function get_todays_keyword($keyword){
		$key = floor(time() / 60 / 60 / 24) % count($keyword);
		return $keyword[$key];
	}


	/**
	 * REST apiで帰ってきたステータスからid部分を獲得する
	 *
	 * @param object $statuses REST apiで取得したステータス
	 * @return array 引数のオブジェクトのidプロパティ
	 */
	public function get_user_ids_from_statuses($statuses){
		return $statuses->ids ? $statuses->ids : array();
	}

	/**
	 * アカウント凍結防止のために、1日のフォロー上限を算出する
	 *
	 * @param string $user_screen_name 算出したいユーザーのスクリーンネーム
	 * @return array 1日のフォロー上限と総計のフォロー上限
	 */
