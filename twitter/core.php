...
class Core {
	 ...
	 *
	 * @param boolean $stringfy_ids 返り値のidを文字列型として扱うか
	 * @param integer $cursor ブロック中ユーザー一覧のページネーションに使う(-1は1ページ目を表す)
	 * @return object ブロック中のユーザーのid一覧
	 */
	public function get_blocks_ids($stringfy_ids = true, $cursor = -1){
		$params_query = array(
			'stringfy_ids'    => $stringfy_ids,
			'cursor'          => $cursor,
		);
		$request_method = 'GET';
		$request_url = 'https://api.twitter.com/1.1/blocks/ids.json';
	...
	/**
	 * ブロック解除する
	 * デフォルトでは返り値にステータス等を含めない
	 *
	 * @param string $user_id ブロック解除する対象ユーザーのid
	 * @param boolean $include_entities 返り値に画像等の詳細情報を含めるか
	 * @param boolean $skip_status 返り値にユーザーステータスの詳細情報を含めるか
	 * @return object 解除した相手ユーザーの情報
	 */
	public function post_blocks_destroy($user_id, $include_entitiies = false, $skip_status = true){
		$params_query = array(
			'user_id'           => $user_id,
		...
		$request_method = 'POST';
		$request_url = 'https://api.twitter.com/1.1/blocks/destroy.json';
		return $this->send_request ($params_query, $request_method, $request_url);
	}
	 ...
	 * フォロー解除する
	 *
	 * @param string $user_id フォロー解除するユーザーのid
	 * @return object フォロー解除したユーザーのステータス
	 */
	 ...
	 * フォロワーの情報を取得する
	 *
	 * @param string $user_screen_name フォロワー情報を取得したい任意のユーザーのスクリーンネーム
	 * @return object フォロワーの一覧
	 */
	 ...
	 * ツイートIDを指定してお気に入りする
	 *
	 * @param string $tweet_id お気に入りしたいツイートのid
	 * @return object お気に入りしたツイートの情報
	 */
	 ...
	 * @param string $lang 検索対象のツイートの言語
	 * @param integer $count 検索する件数
	 * @return object 検索結果の一覧
	 */
	 ...
	 * フォローしている人のデータを取得する
	 *
	 * @return object フォローしているユーザーid一覧
	 */
	 ...
	 * ダイレクトメッセージを送る
	 *
	 * @param string $receiver_screen_name 送信先ユーザーのスクリーンネーム
	 * @param string $msg 送信するDMの本文
	 * @return object 送信されたDMの情報
	 */
	 ...
	 * 特定のユーザーの情報を取得する
	 *
	 * @param string $user_screen_name 取得したいユーザーのスクリーンネーム
	 ...
	 *フォロワーのID一覧を取得する
	 *
	 *@param string $user_screen_name フォロワー一覧を取得したいユーザーのスクリーンネーム
	 *@return object フォロワーのid一覧
	 */
	 ...
	 *
	 * @param string $user_id フォローする相手のユーザーid
	 * @return object フォローした相手ユーザーの情報
	 */
	 ...
	 *
	 * @param array $params_query リクエストに付加するパラメータ
	 * @param string $request_method リクエスト方式
	 * @param string $request_url リクエスト先のurl
	 * @param array $app_keys OAuth認証用パラメータ
	 * @param boolean $as_json リクエスト結果をjson_decodeするか
	 * @return mixed リクエスト結果
	 */
